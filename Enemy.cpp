...
void Enemy::Update()
{
	...
	if (enemyIsAlive_ == false) //“G‚ÌŒ‚”jŽž
	{
		enemyRespawnCount_ -= 1;
		enemyExplosion_ = true;
	}
	if (enemyRespawnCount_ <= 90) //0.5•bŒã”š”­‰æ‘œ‚ðÁ‚·
	{
		enemyExplosion_ = false;
	}
	if (enemyRespawnCount_ == 0) //“GŒ‚”j‚Ì“ñ•bŒã‚É•œŠˆ
	{
		enemyIsAlive_ = true;
		enemyRespawnCount_ = 120;
	}

	if (enemyBullet_.GetIsEnemyBulletShot() == false && enemyIsAlive_ == true && delayFrameBulletEnemy_ <= 0) //“G‚Ì’e‚Ì”­ŽË
	{
		delayFrameBulletEnemy_ = enemyDelayFrameBullet_;
...
void Enemy::Move()
{
	if (enemyIsAlive_ == true)
	{
		...
		if (enemyPosY_ + enemyRadius_ >= 720) //“G‚Ì”½ŽË
		{
			enemySpeedY_ *= -1;
		}
